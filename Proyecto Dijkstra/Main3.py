...
MENU_TITLE_IMG = os.path.join(ASSETS_DIR, "Titulo.png")  # imagen que subiste
...
# colores solicitados
MENU_BTN_BG = "#0260A8"
MENU_BTN_BORDER = "#87F5FF"
MENU_BTN_TEXT = "#87F5FF"
ALGO_BTN_BG = "#02101D"
ALGO_BTN_TEXT = "#87F5FF"
...
FIXED_NODES = [
    ("Torre 1", 236, 335),  ("Torre 2", 390, 230),  ("Torre 3", 430, 360),
...
class DijkstraApp:
    ...
    def _load_resources(self):
        ...
        # cargar imagen de título del menú si existe
        if os.path.exists(MENU_TITLE_IMG):
            try:
                img = Image.open(MENU_TITLE_IMG).convert("RGBA")
                # conservar tamaño proporcional del texto original: escalar a ~560x64 si es mucho mayor
                max_w, max_h = 760, 120
                iw, ih = img.size
                scale = min(1.0, max_w / iw, max_h / ih)
                tw, th = int(iw * scale), int(ih * scale)
                img = img.resize((tw, th), Image.LANCZOS)
                self._menu_title_tk = ImageTk.PhotoImage(img)
            except Exception:
                self._menu_title_tk = None
        else:
            self._menu_title_tk = None
    ...
    # -----------------------------
    # Menú principal (usa imagen de título si está)
    # -----------------------------
    def _build_menu(self):
        ...
        # colocar la imagen del título si se cargó; si no, usar texto fallback
        if self._menu_title_tk:
            title_widget = tk.Label(overlay_parent, image=self._menu_title_tk, bg="#001523", bd=0)
            # mantener tamaño y contorno visual similar al texto original; center
            title_widget.place(relx=0.5, rely=0.18, anchor="center")
        else:
            font_title = (self.custom_font_family, 20, "bold") if self.custom_font_family else ("Segoe UI", 20, "bold")
        ...
        # botones del menú con colores pedidos
        btn_cfg = {
            "bg": MENU_BTN_BG,
            "fg": MENU_BTN_TEXT,
            "activeforeground": MENU_BTN_TEXT,
            "activebackground": MENU_BTN_BG,
            "bd": 2,
            "relief": "solid",
            "highlightthickness": 0,
        }
        # aplicar contorno estableciendo highlightbackground/highlightcolor para algunos sistemas
        # y usando relief+bd; depende de plataforma cómo se vea exactamente
        b1 = tk.Button(overlay_parent, text="A PIE", command=lambda: self._start_transition_and_open('pie'), **btn_cfg)
        b2 = tk.Button(overlay_parent, text="PARAVELA", command=lambda: self._start_transition_and_open('paravela'), **btn_cfg)
        b3 = tk.Button(overlay_parent, text="CABALLO", command=lambda: self._start_transition_and_open('caballo'), **btn_cfg)
        # aplicar estilo adicional (bordes) si la plataforma lo permite
        for b in (b1, b2, b3):
            try:
                b.configure(highlightbackground=MENU_BTN_BORDER, highlightcolor=MENU_BTN_BORDER)
            except Exception:
                pass
        b1.place(relx=0.32, rely=0.4, anchor="center", width=220, height=54)
        b2.place(relx=0.5, rely=0.4, anchor="center", width=220, height=54)
        b3.place(relx=0.68, rely=0.4, anchor="center", width=220, height=54)

        hint_font = (self.custom_font_family, 10) if self.custom_font_family else ("Segoe UI", 10)
    ...
    def _build_main(self):
        ...
        # fuentes y estilos de botones del panel algoritmo (fondo #02101D)
        ...
        algo_btn_cfg = {
            "bg": ALGO_BTN_BG,
            "fg": ALGO_BTN_TEXT,
            "activebackground": ALGO_BTN_BG,
            "activeforeground": ALGO_BTN_TEXT,
            "bd": 0,
            "relief": "flat",
            "font": font_btn
        }
        tk.Button(left, text="Reiniciar aristas", command=self._reset_edges, **algo_btn_cfg).pack(fill="x", padx=12, pady=6)
        tk.Button(left, text="Seleccionar inicio/destino", command=self._set_select_start_end, **algo_btn_cfg).pack(fill="x", padx=12, pady=6)
        tk.Button(left, text="Ejecutar Dijkstra", command=self._on_execute_dijkstra, **algo_btn_cfg).pack(fill="x", padx=12, pady=6)
        tk.Button(left, text="Volver al menú", command=self._back_to_menu, **algo_btn_cfg).pack(fill="x", padx=12, pady=6)
    ...
    def _start_counter_animation(self, target, duration_ms):
        if self._counter_after_id:
            try:
                self.root.aftercancel(self._counter_after_id)
            except:
                pass
            self._counter_after_id = None
        self._counter_current = 0.0
